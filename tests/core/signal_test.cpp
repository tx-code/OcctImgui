#define BOOST_TEST_MODULE Core Signal Tests
...
BOOST_AUTO_TEST_CASE(scoped_connection_test)
{
    ...
    // Act - create a scope
    {
        ScopedConnection connection(signal.connect([&](int) {
            callCount++;
        }));
        signal.emit(1);
        BOOST_CHECK_EQUAL(callCount, 1);
...
BOOST_AUTO_TEST_CASE(scoped_connection_move_test)
{
    ...
    // Act
    ScopedConnection connection1(signal.connect([&](int) {
        callCount++;
    }));
...
BOOST_AUTO_TEST_CASE(connection_tracker_test)
{
    ...
    // Act
    tracker.track(signal1.connect([&](int) {
        signal1CallCount++;
    }));
    tracker.track(signal2.connect([&](const std::string&) {
        signal2CallCount++;
    }));
...
BOOST_AUTO_TEST_CASE(connection_tracker_direct_signal_tracking_test)
{
    ...
    tracker.track(signal.connect([&](int) {
        callCount++;
    }));
    signal.emit(1);
