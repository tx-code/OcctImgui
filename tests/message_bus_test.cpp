...
BOOST_AUTO_TEST_CASE(message_bus_selection_info_test)
{
    ...
    bus.subscribe(MessageBus::MessageType::SelectionChanged, [&](const MessageBus::Message& message) {
    ...
    BOOST_CHECK(receivedInfo.subFeatures[objectId][0].type == MessageBus::SelectionInfo::SubFeatureType::Face);
...
BOOST_AUTO_TEST_CASE(message_bus_callable_objects_test)
{
    ...
    struct MessageHandler {
        ...
        MessageHandler(int& c) : counter(c) {}
    ...
    public:
        ...
        Observer(int& count) : memberFunctionCount(count) {}
    };
    
    Observer observer(memberFunctionCount);
    bus.subscribe(MessageBus::MessageType::ModelChanged, [&observer](const MessageBus::Message& msg) {
...
BOOST_AUTO_TEST_CASE(message_bus_callable_state_capture_test)
{
    ...
    for (int i = 0; i < 3; i++) {
        // 每个 lambda 捕获不同的 i 值
        bus.subscribe(MessageBus::MessageType::CommandExecuted, [i, &values](const MessageBus::Message&) {
...
BOOST_AUTO_TEST_CASE(message_bus_complex_callable_test)
{
    ...
    bus.subscribe(MessageBus::MessageType::ModelChanged, [&dataMap](const MessageBus::Message& message) {
        if (message.data.type() == typeid(int)) {
            dataMap["int"] = std::any_cast<int>(message.data);
        } else if (message.data.type() == typeid(std::string)) {
            dataMap["string"] = static_cast<int>(std::any_cast<std::string>(message.data).length());
